...
class User
{
    ...
    public function forgotPassword($email)
    {
        ...
        if ($this->isUser())
        {
            ...
            {
                $user_data = get_user_by('email', trim($this->useremail));
                if ( empty($user_data) )
                {
                    $this->setErrors(array(8 => 'There is no user registered with that email address'));
                    return $this;
            ...
            $message = __('Кто-то запрашивает смену пароля на сайте ФК Нива Чернигов.') . "\r\n\r\n";
            $message .= network_site_url() . "\r\n\r\n";
            $message .= sprintf(__('Пользователь: %s'), $user_login) . "\r\n\r\n";
            $message .= __('Что бы изменить пароль, перейдите по ссылке или же просто проигнорируйте данное письмо и ничего не произойдет.') . "\r\n\r\n";
            $message .= network_site_url("account/reset-password/?action=rp&key=$key&login=" . rawurlencode($user_login), 'login') . "\r\n";
            $blogname = wp_specialchars_decode(get_option('blogname'), ENT_QUOTES);
            $title = sprintf( __('[%s] Сменить пароль'), $blogname );
    ...
    {
        $user_data = get_user_by('login', $data['login']);
    ...
    protected function isUserApproved()
    {
        global $wpdb;
        $user = get_user_by('email', $this->useremail);
        $query = "SELECT post_id FROM $wpdb->postmeta WHERE meta_key = 'user_account' AND meta_value = $user->ID";
        $approved = $wpdb->get_var($query);
        return $approved;
    }
    ...
    protected function createValidation($fields)
    {
        ...
        if ($fields['edit'] !== 'true')
        {
            ...
            $this->new_user_id = get_user_by('email', $fields['user_email']);
            if ($this->new_user_id)
            {
    ...
    protected function sendRegConfirmation()
    {
        $subject = 'Запрос на создание аккаунта';

        $message = __('Подтверждение аккаунта на сайте:') . "\r\n\r\n";
        $message .= network_site_url() . "\r\n\r\n";
        $message .= __('Для подтверждения аккаунта перейдите по ссылке.') . "\r\n\r\n";

        $headers[] = 'From: '.get_option('blogname').' <info@niva.cn.ua>';
